...
do
  if [[ ! $value =~ "b:" ]]; then
    ...
    echo "::notice::Pushing changes for release branch $release_branch"
    ...
    if [ "$DRY_RUN" = "true" ]; then
      echo "::debug::Start dry-run for pushing changes for release_branch: $release_branch"
      git push --dry-run --set-upstream origin "$release_branch"
      echo "::notice::Finished dry-run for pushing changes for release branch: $release_branch"
    else
      echo "::debug::Start pushing changes for release_branch: $release_branch"
      git push --set-upstream origin "$release_branch"
      echo "::debug::Finished pushing changes for release branch: $release_branch"
    fi
  fi
done
